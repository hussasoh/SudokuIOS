...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    var window: UIWindow?
    var databaseName : String? = "ProjectDatabase.db"   // Name of the database
    var databasePath : String?                          // Path of the database
    ...
    // Upon launch of application firebase is configured, database is checked/created, and data is read from database
    ...
    func readDataFromDatabase() {
        ...
        players.removeAll()
        var db : OpaquePointer? = nil       // our database pointer
        ...
        if sqlite3_open(self.databasePath, &db) == SQLITE_OK {
            ...
            if sqlite3_prepare_v2(db, queryStatementString, -1, &queryStatement, nil) == SQLITE_OK {
                ...
                while sqlite3_step(queryStatement) == SQLITE_ROW {
                    // preparing statement, connecting the database row colomns and assigning them to new variables
                    ...
                    let data : Player = Player.init()   // player is initalized
                    
                    // the variables (data) are inserted into our Player data holder class
                    data.initWithData(theRow: id, theName: name, theScore: Int(score))
                    
                    // this is then added to our players array which holds all our data
                    players.append(data)
    ...
    func insertIntoDatabase(player : Player) -> Bool {
        var db : OpaquePointer? = nil // pointer of database
        var returnCode : Bool = true  // return code which will tell us if the insertion was successful
        
        if sqlite3_open(self.databasePath, &db) == SQLITE_OK {
            
            var insertStatement : OpaquePointer? = nil
            let insertStatementString : String = "insert into users values(NULL, ?, ?)"  // the string that we will use to insert to                                                                           // database
